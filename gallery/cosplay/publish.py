"""Publish the COSPLAY gallery: optimize, stage and push, in one pass.

    python gallery/cosplay/publish.py --source <dir>              # add missing only
    python gallery/cosplay/publish.py --source <dir> --overwrite  # also replace existing
    python gallery/cosplay/publish.py --source <dir> --dry-run    # report, change nothing
...
THE SAFETY MODEL, which is the whole point of this script:
  ...
  * Nothing is removed unless you pass ``--prune-orphans``, and even then only
    images whose filename matches no roster entry at all - i.e. a character that
...
GALLERY_TITLE = "cosplay character"
